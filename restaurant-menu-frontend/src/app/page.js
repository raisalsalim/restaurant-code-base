...
export default function Home() {
  ...
  useEffect(() => {
    axios
      .get("http://4.186.9.79:8000/api/menu-items/") // Adjust the URL to match your backend
      .then((response) => {
  ...
  const handleCreate = () => {
    axios
      .post("http://4.186.9.79:8000/api/menu-items/", newItem) // Sending new item to backend
      .then((response) => {
  ...
  const handleUpdate = () => {
    axios
      .put(`http://4.186.9.79:8000/api/menu-items/${editItem.id}/`, editItem) // Sending updated item
      .then((response) => {
  ...
  const handleDelete = (id) => {
    axios
      .delete(`http://4.186.9.79:8000/api/menu-items/${id}/`) // Deleting item
      .then(() => {
